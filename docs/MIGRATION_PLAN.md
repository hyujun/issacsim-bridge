...
2. 호스트 유닛 테스트 상태 확인:
   ```bash
   cd isaac_scripts && pytest sim_bridge/tests/ -v --ignore=sim_bridge/tests/integration
   # 22/22 통과해야 정상
   ```
...
- 선행 조건: 없음
- 착수: [isaac_scripts/sim_bridge/](../isaac_scripts/sim_bridge/) 에 `ui_panel.py` 신설 → `omni.ui.Window` + `robot.yaml::gui.buttons` 기반 동적 버튼. 상세는 본 문서 "Phase 3" 섹션 (체크리스트 포함).
...
- smoke test: 기존 [run_smoke.sh](../isaac_scripts/sim_bridge/tests/integration/run_smoke.sh) 에 새 env flag (`SIM_BRIDGE_MODE=omnigraph|rclpy`) 추가해서 양쪽 경로 비교.
...
- **ROS bridge 는 여전히 rclpy sidechannel** — [sim_bridge/ros_bridge.py](../isaac_scripts/sim_bridge/ros_bridge.py) 의 `setup_rclpy_bridge()`. OmniGraph 네이티브 전환은 Phase 1.3 에서 수행.
- **Articulation 경로** 는 여전히 Newton tensor ArticulationView — [sim_bridge/newton_view.py](../isaac_scripts/sim_bridge/newton_view.py). `isaacsim.core.experimental.prims.Articulation` 전환은 Phase 1.4 에서 수행 (1.3 결과에 따라 필요 여부 결정).
- **테스트 인프라**: 22 host unit tests (`sim_bridge/tests/test_config.py`, `test_dof_map.py`) + docker smoke harness (`sim_bridge/tests/integration/`). `pytest -m phase12` 로 per-patch 회귀 검증 재실행 가능.
...
1. **`isaacsim.core.experimental.prims.Articulation`** — Newton 네이티브 대응 Articulation 래퍼.
   ...
   - **우리 측 [find_articulation_root_path()](../isaac_scripts/sim_bridge/robot.py#L51-L69) + Newton ArticulationView 래핑 전체를 대체 가능**.
...
목표: [isaac_scripts/sim_bridge/usd_patches.py](../isaac_scripts/sim_bridge/usd_patches.py) 의 4 개 함수 각각이 URDFImporter 3.2.1 출력에서 여전히 필요한지 재확인.

#### 진행 절차

1. pytest 테스트 인프라 구축: [isaac_scripts/sim_bridge/tests/](../isaac_scripts/sim_bridge/tests/) 신설.
2. [sim_bridge/config.py](../isaac_scripts/sim_bridge/config.py) 를 순수 함수 기반으로 리팩터 (lazy module-level constants 로 하위 호환 유지).
3. [sim_bridge/dof_map.py](../isaac_scripts/sim_bridge/dof_map.py) 신설 — Newton 의존성 없는 DOF name ↔ index 매핑 순수 함수.
4. [launch_sim.py](../isaac_scripts/launch_sim.py) 에 `SIM_HEADLESS` / `SIM_SKIP_PATCHES` / `SIM_MAX_RUN_SECONDS` 환경 변수 추가.
5. [tests/integration/run_smoke.sh](../isaac_scripts/sim_bridge/tests/integration/run_smoke.sh) — `docker compose run --rm` 래퍼로 헤드리스 smoke test 실행.
6. [tests/integration/test_phase1_2_usd_patches.py](../isaac_scripts/sim_bridge/tests/integration/test_phase1_2_usd_patches.py) — 4 개 per-patch smoke test harness.

#### 유닛 테스트 결과

| 대상 | 테스트 수 | 결과 |
|---|---|---|
| [tests/test_config.py](../isaac_scripts/sim_bridge/tests/test_config.py) | 12 | ✅ 12/12 |
| [tests/test_dof_map.py](../isaac_scripts/sim_bridge/tests/test_dof_map.py) | 10 | ✅ 10/10 |
| **합계** | **22** | **✅ 22/22** |
...
- [sim_bridge/usd_patches.py](../isaac_scripts/sim_bridge/usd_patches.py) 에서 `strip_zero_mass_api` / `populate_robot_schema_links` 함수 삭제. 모듈 docstring 에 제거 사유 기록.
...
목표: [isaac_scripts/sim_bridge/ros_bridge.py](../isaac_scripts/sim_bridge/ros_bridge.py) 의 rclpy sidechannel 을 `IsaacReadJointState` (sensor) → `ROS2PublishJointState` (sensor-input) OmniGraph 그래프로 교체. `/joint_command` 측은 `ROS2SubscribeJointState` + `IsaacArticulationController` 조합 또는 sensor-input 기반 대안으로 구성.
...
5. 두 경로 모두 통과 시 [main_loop.py](../isaac_scripts/sim_bridge/main_loop.py) 의 `apply_cmd` / `publish_state` / rclpy spin 제거. 루프는 `world.step` + `simulation_app.update` 만.
...
- [ ] sync 모드 호환 검증
- [ ] [ros_bridge.py](../isaac_scripts/sim_bridge/ros_bridge.py) / [main_loop.py](../isaac_scripts/sim_bridge/main_loop.py) 에서 rclpy sidechannel 제거
...
목표: [newton_view.py](../isaac_scripts/sim_bridge/newton_view.py) 의 Newton tensor ArticulationView 를 `isaacsim.core.experimental.prims.Articulation` 로 교체. `isaacsim.core.experimental.prims.Articulation.fetch_articulation_root_api_prim_paths()` 가 내부적으로 ArticulationRootAPI 를 탐색하므로 [find_articulation_root_path()](../isaac_scripts/sim_bridge/robot.py#L51-L69) 도 제거.
...
목표: `robot.yaml` 에 `sensors:` 섹션 추가 → [sim_bridge/](../isaac_scripts/sim_bridge/) 에 `sensors.py` 신설 → pack 별로 contact/IMU/camera 센서를 자동 부착 + ROS2 publish.
...
- [ ] `sim_bridge/sensors.py` 구현 (contact · IMU · camera 최소 3종)
...
목표: [robot.py::build_world()](../isaac_scripts/sim_bridge/robot.py#L13-L40) 의 `add_default_ground_plane()` 하드코딩을 "베이스 씬 USD 레이어" 패턴으로 분리. 로봇은 sublayer composition 으로 올림.
...
- [ ] `sim_bridge/robot.py::build_world()` 에 sublayer composition 로직 추가
...
- [ ] `sim_bridge/ui_panel.py` 스켈레톤 + Window 등록 확인
